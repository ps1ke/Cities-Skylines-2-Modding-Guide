# Game.UI.InGame.CityInfoPanel

**Assembly:** `Game`  
**Namespace:** `Game.UI.InGame`  

**Type:** class public  

**Base:** `Game.UI.InGame.TabbedGamePanel`  
**Implements:** `Colossal.UI.Binding.IJsonWritable`, `System.IEquatable<Game.UI.InGame.TabbedGamePanel>`  

## Code

```csharp
public class CityInfoPanel : Game.UI.InGame.TabbedGamePanel, Colossal.UI.Binding.IJsonWritable, System.IEquatable<Game.UI.InGame.TabbedGamePanel>
{
    public System.Boolean blocking { get; }
    public Game.UI.InGame.GamePanel+LayoutPosition position { get; }

    public CityInfoPanel();

}
```


## Properties

- `public System.Boolean blocking { get }`  

```csharp
public System.Boolean blocking { get; }
```

- `public Game.UI.InGame.GamePanel+LayoutPosition position { get }`  

```csharp
public Game.UI.InGame.GamePanel+LayoutPosition position { get; }
```


## Constructors

- `public CityInfoPanel()`  

```csharp
public CityInfoPanel();
```


## Nested types

- `Game.UI.InGame.CityInfoPanel+Tab`  

